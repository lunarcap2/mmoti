...
function fn_calculate() {
	...
	var underage_cnt =  parseInt($("#underage_children").val().split(',').join(''));	// 20세이하 자녀수
	if (underage_cnt == 0)
		$("#underage_children").val(0);
	...
	*/
	...
	else if (monthly_salary > 10000000 && monthly_salary <= 14000000)
	{
		annual_salary = monthly_salary*12;	// 급여가 천만원 초과, 1400 이하 = 천만원 해당 세액 + 천만원을 초과하는 금액 중 98% 곱한 금액의 35% 상당액
	}
	
	else if(monthly_salary > 14000000)
	{
		// 급여 1400 초과
		annual_salary = monthly_salary*12;	// 급여가 1400만원 초과 = 천만원 해당 세액 + 1372000 + 14000000 초과하는 금액 중 98% 곱한 금액의 38% 상당액
	}
	else {
		annual_salary = monthly_salary*12;	// 급여가 천만원이 넘으면 급여액에 12를 곱한금액을 지정한다
	}
    ...
    // alert(' period_min: ' + period_min);
	// alert(' period_max : ' + period_max);
	...
	//alert("근로소득공제액 : " + earned_deduction  ); // 12,156,000 절사
	...
	if (annual_salary <= 30000000)
	{
		...
		if ( dependency == 1 )
		{
			special_deduction = 3100000 + Math.floor (annual_salary * 4 / 100) ; // 310만원 + 연간 총급여액의 4%
		}
		...
		else if( dependency >= 3)
		{
			...
			*/
			special_deduction = 5000000 + Math.floor(annual_salary * 7 /100 ) ;
		}
	}
	...
	{
		// 공제대상자 1명인 경우
		// 310만원 + 연간 총급여액의 4% - 연간 총급여액 중 3천만원 초과 금액의 5%
		if ( dependency == 1 )
		{
			special_deduction = 3100000 + Math.floor( annual_salary *4/100) - Math.floor(( annual_salary - 30000000 ) * 5/100) ; // 310만원 + 연간 총급여액의 4%
		}
		...
		{
			special_deduction = 3600000 + Math.floor( annual_salary *4/100) - Math.floor(( annual_salary - 30000000 ) * 5/100) ; // 310만원 + 연간 총급여액의 4%
		}
		else if( dependency >= 3)
		{
			/*
			//공제대상자 3명 이상인 경우 500만원 + 연간 총급여액의 7% - 연간총급여액 중 3천만원을 초과하는 금액의 5%
			// (60,120,000원×5%)+[(60,120,000원-40,000,000원) ×4%]
			*/
			if (annual_salary > 40000000)
			{
				...
				special_deduction = 5000000 + Math.floor(annual_salary *7 /100 )  - Math.floor( ( annual_salary - 30000000) * 5/100)  ;
			}
	...
	else if (annual_salary > 45000000 && annual_salary <= 70000000 )
	{
		...
		if ( dependency == 1 )
		{
			special_deduction = 3100000 + Math.floor( annual_salary * 1.5 / 100);
		}
	...
	else if (annual_salary > 70000000 && annual_salary <= 120000000)
	{
		...
		if (dependency == 1)
		{
			special_deduction = 3600000 + Math.floor( annual_salary * 50/100);
		}
		// 공제대상자 2명인 경우
		// 360만원 + 연간 총급여액의 1%
		else if ( dependency == 2 )
	...
	*/
	...
	} else if (assessment_standard <= 150000000 )
	{
		...
		calculated_tax =  37600000 + (assessment_standard - 150000000) * 0.38  ;
	}
	...
	// 산출세액이 50초과 = 275000 + 50만원 초과하는 금액의 100/30
	// 산출세액이 50이하 = 산출세액의 * 100/55
	...
	//alert("근로소득세액공제액 버림  : " + Math.floor(earned_income_tax_deduction) ); 

	// 결정세액 (산출세액 - 근로소득세액공제액)
	//var settled_tax = Math.floor(calculated_tax - earned_income_tax_deduction);
	var settled_tax =  calculated_tax -  Math.floor(earned_income_tax_deduction);
	...
	} else if (monthly_salary >= 220000 && monthly_salary < 4210000) {
		national_pension = fn_cut_off(Math.floor(monthly_salary * 4.5 / 100), 10);	// 월급여액(비과세액 제외) 4.5%
	} else {
		national_pension = fn_cut_off(Math.floor(4210000 * 4.5 / 100), 10);
	}
	*/
	...
	* 주의) 상한액은 국민연금 전체가입자의 평균소득을 3년간 평균한 값에 연동하여 매년 7월 변동됩니다.   즉 국민연금공단에서 인상을 추진하려는 의도가 아니라 이미 정해진 규약입니다.   따라서 국민연금 요율 인상과는 전혀 무관합니다.
	...
	기준소득월액이란 ? 
국민연금의 보험료 및 급여 산정을 위하여 가입자가 신고한 소득월액에서 천원 미만을 절사한 금액을 말하여, 최저 27만원에서 최고금액은 421만원까지의 범위로 결정하게 됩니다. 따라서, 신고한 소득월액이 27만원보다 적으면 27만원을 기준소득월액으로 하고, 421만원보다 많으면 421만원을 기준소득월액으로 합니다.

	*/

	if (monthly_salary < 270000) {
		national_pension = fn_cut_off(Math.floor(270000 * 4.5 / 100), 10);
	} else if (monthly_salary >= 270000 && monthly_salary < 4210000) {
		national_pension = fn_cut_off(Math.floor(monthly_salary * 4.5 / 100), 10);	// 월급여액(비과세액 제외) 4.5%
	} else {
		national_pension = fn_cut_off(Math.floor(4210000 * 4.5 / 100), 10);
	}
	...
	//alert("기준소득월액  : " + monthly_salary );
	//alert("국민연금  : " + national_pension );
	...
	*/
	...
	var health_insurance = 0;
	if (monthly_salary < 280000) {
		health_insurance = fn_cut_off(Math.floor(280000 * 3.06 / 100 ), 10);
	} else if (monthly_salary >= 280000 && monthly_salary < 65790000) {
		health_insurance = fn_cut_off(Math.floor(monthly_salary * 3.06 / 100 ), 10);	// 월급여액(비과세액 제외) 2.82%
	} else {
		health_insurance = fn_cut_off(Math.floor(65790000 * 3.06 / 100 ), 10);
	}
	$("#health_insurance").val(health_insurance);
	...
	$("#expected_receipt").text( commify(expected_receipt) );
...
*/
function fn_reset() {
	$("#annual_salary").attr("checked",true);
	$("#dependency").val("1");
	fn_chk_dependency();
	
	$("#rg01").attr("checked",true);
	...
	$("#expected_exemption").text("0");
	$("#income_salary").val("");
	$("input[name=salary_type]").eq(0).click();
	$("#korean_num").text("");

	$("#national_pension").val("");
	$("#health_insurance").val("");
	$("#longterm_insurance").val("");
	$("#unemployment_insurance").val("");
	$("#income_tax").val("");
	$("#inhabitants_tax").val("");
	$("#total_exemption").val("");
	$("#expected_receipt").text("0");
}
...
function fn_chk_dependency(obj) {
	...
	if (fn_chk_numeric_Salary(obj)) {
		var dependency = parseInt(obj.val());
		if(dependency ==1) {
			$("#children01").attr("checked",true);
...
*	팁 노출
*/
function fn_disp_tip(i) {
	$(".terminology dl").each(function(idx, item) {
		$(this).attr("class", (i==idx) ? "active" : "default");
		if(i==idx) {
